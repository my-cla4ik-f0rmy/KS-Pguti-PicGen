...
function post_sm(){
  ...
  pg.text(name2, 50, 300, 707);
  pg.textLeading(70);
  pg.textFont(font2);
  pg.textSize(Test.fontsize3)
  pg.image(logo, 895, 895, 101, 115);
  image(pg, 0, 0, width, height);
}
...
function post_sn(){
  const name = document.getElementById("sample5").value;
  const name2 = document.getElementById("sample6").value;
  const test_slider = document.getElementById("volume").value;
  znach = document.querySelector('input[name="options"]:checked').value

  if(znach==1){
    pg.background('#63B0DE')
  } else if(znach==2){
    pg.background('#E87156')
  } else if(znach==3){
    pg.background('#27C679')
  }
  pg.fill('ffffff');
  pg.textFont(font3);
  pg.textSize(Test.data.fontsize4)
  pg.textWrap(WORD);
  pg.textLeading(100);
  pg.textAlign(LEFT, LEFT);
  pg.text(name, 80, 140, 920);

  pg.textFont(font);
  pg.textSize(Test.data.fontsize5)
  pg.textWrap(WORD);
  pg.textLeading(70);
  pg.textAlign(LEFT, LEFT);
  pg.text(name2, 80, test_slider*1, 920);
